...
void GPIO::GenerateVariablePWM(int period, int first_pulse, int last_pulse, int num_periods) {
	GeneratePWM(period, first_pulse, 50);
	int step =((last_pulse - first_pulse)/num_periods); 
	for (int i = 0; i < num_periods; i++)
	{
		write(fd, "1", 1);
		usleep(first_pulse + (i * step));
		write(fd, "0", 1);
		usleep(period - (first_pulse + (i*step)));
	}
	GeneratePWM(period, last_pulse, 50);
}
